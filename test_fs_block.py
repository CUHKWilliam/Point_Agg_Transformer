...
def do_test(model, dataset):
    ...
    with torch.no_grad():
        ...
        for i, batch_input in enumerate(dataloader):
            ...
            class_inds = query_dict["labels"].detach().cpu().numpy()
            instance_inds = query_dict["instance_labels"].detach().cpu().numpy()
            mask = np.logical_or(instance_inds == -100, class_inds == -100)
            class_inds[mask] = 0
            class_inds = np.array(BENCHMARK_SEMANTIC_LABELS)[class_inds]
            class_inds[mask] = 0
            instance_inds[mask] = 0
            gt_inds = class_inds * 1000 + instance_inds

            mask = np.any((gt_inds // 1000)[:, None] == np.array(np.array(BENCHMARK_SEMANTIC_LABELS)[active_label])[None, :], axis=-1)
            gt_inds[np.logical_not(mask)] = 0
