...
class GeoFormerFS(nn.Module):
            ...
            for b in range(batch_size):
                ...
                if corr_save_path is not None:
                    visualize_pts_open3d(context_locs_b.squeeze(0),
                                         filename=os.path.join(corr_save_path, "sampled_support_{}.ply".format(b)))
                    visualize_pts_open3d(locs_float_[start:end, :],
                                         filename=os.path.join(corr_save_path, "support_masked_{}.ply"))
            ## TODO: self-alignment
            support_embeddings = torch.cat(support_embeddings, dim=0)  # batch x channel
            coords_float = torch.cat(coords_float, dim=0)
            coords__ = torch.cat(coords__, dim=0)
    ...
    def forward(self, support_dict, scene_dict, training=True, remember=False,
        ...
        if show_corr:
            ...
            supp_coords = torch.unique(corr_coords_b[:, -3:], dim=0)
            for supp_coord in supp_coords:
                selected = (corr_coords_b[:, -3:] == supp_coord).all(-1)
                coords_sel = corr_coords_b[selected][:, :3].float()
