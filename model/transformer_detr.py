...
from model.helper import ACTIVATION_DICT, NORM_DICT, WEIGHT_INIT_DICT, get_clones
...
class TransformerEncoderLayer(nn.Module):
    def __init__(
        ...
        if self.use_ffn:
            ...
            self.linear1 = \
                nn.Sequential(
                    nn.Linear(d_model, dim_feedforward, bias=ffn_use_bias),
                    nn.ReLU(),
                )
...
class CrossAttention(nn.Module):
    ...
    def forward(self, tgt, src, pos):
        n_queries, n_context, batch, channel = pos.shape
        tgt_expand = tgt[:, None, :, :].repeat(1, n_context, 1, 1)

        src_expand = src[None, :, :, :].repeat(n_queries, 1, 1, 1)

        sim = self.attn_mlp(tgt_expand - src_expand + pos)
        attn = F.softmax(sim / np.sqrt(sim.shape[-1]), dim=1)

        v2 = self.v_mlp(src + pos)
        tgt = torch.einsum("qcbf,qcbf->qbf", attn, v2)  # n_queries, batch, channel
        tgt = self.out_mlp(tgt)
        return tgt
