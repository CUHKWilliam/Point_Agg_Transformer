...
class FSInstDataset:
    def __init__(self, split_set="train"):
        ...
        self.SEMANTIC_LABELS = [3]  # bed
        self.SEMANTIC_LABELS_MAP = {val: (idx + 4) for idx, val in enumerate(self.SEMANTIC_LABELS)}

        class2scans_file = os.path.join(self.data_root, self.dataset, "class2scans_block.pkl")
    ...
    def load_single(self, scene_name, aug=True, permutate=True, val=False, support=False):
        ...
        if not support:
            data = np.load(os.path.join(self.data_root, self.dataset, "scenes/blocks_bs3_s3/data", "%s.npy" % scene_name))
        else:
            data = np.load(
                os.path.join(self.data_root, self.dataset, "scenes", "%s.npy" % scene_name))
